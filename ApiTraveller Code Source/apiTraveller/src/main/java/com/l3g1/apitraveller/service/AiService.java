...
import com.l3g1.apitraveller.dto.AiRequest;
import com.l3g1.apitraveller.dto.AiResponse;
...
public class AiService {
    ...
    private static String temperatureValues = "temperature: [\"HOT\", \"WARM\", \"MILD\", \"COOL\", \"COLD\"]]";
    ...
    public String chatCountry(Survey survey) {
        ...
        if (survey.getLocalisation().equals("ALL")|| Continent.isValidValue(survey.getLocalisation().toUpperCase())) {
            if(survey.getLocalisation().equals("ALL")) {
                ...
                listCountry = countryRepository.findAllByContinent( Continent.valueOf(survey.getLocalisation()));
                promptBuilder.append(" which is not in the following list : [");
                for (Country country : listCountry){
                    promptBuilder.append(" \"" + country.getCountryName() + "\", ");
                }
            ...
            if(!survey.getClimate().equals(Climate.ALL)){
                ...
                promptBuilder.append(" which is not in the following list : [");
                listCountry = countryService.findAllByClimateListContaining(survey.getClimate());
                for (Country country : listCountry){
                    promptBuilder.append(" \"" + country.getCountryName() + "\", ");
                }
            }
        ...
        promptBuilder.append(". Give me the result in the following format, complete the data, price is an attribute with the number only, it's an Integer, consider the currency is the Dollar, :");
        promptBuilder.append("{\n");
        ...
        promptBuilder.append("            \"temperatureList\": [\"\"],\n");
    ...
    public String chatActivity(Country country, City city, List<Activity> listActivity, Survey survey) {
        ...
        promptBuilder.append("]");
        promptBuilder.append(". Here are the possible values for the elements in temperatureList : "+temperatureValues);
        promptBuilder.append(". Give me the result in the following format, complete the data, price is an attribute with the number only, it's an Integer, consider the currency is the Dollar, :");
        promptBuilder.append("{\n");
    ...
    private String processAiMessage(String aiMessage) {
        ...
        List<String> validTemperatureValues = Arrays.asList("HOT", "WARM", "MILD", "COOL", "COLD");
        ...
        try {
            ...
            if (messageJson.has("cityList")) {
                ...
                for (JsonNode cityNode : cityList) {
                    ...
                    if (cityNode.has("temperatureList")) {
                        // Get the list of transports for the current city
                        ArrayNode temperatureList = (ArrayNode) cityNode.get("temperatureList");
                        ArrayNode validTemperatureList = mapper.createArrayNode();

                        // Iterate through the values of transportList
                        for (JsonNode temperatureNode : temperatureList) {
                            String temperatureValue = temperatureNode.asText();
                            // Check if the transport value is valid, sinon met Other
                            if (validTemperatureValues.contains(temperatureValue.toUpperCase())) {
                                validTemperatureList.add(temperatureValue.toUpperCase());
                            } else {
                                validTemperatureList.add("OTHER");
                            }
                        }

                        // Replace the transportList with the new valid list
                        ((ObjectNode) cityNode).set("temperatureList", validTemperatureList);
                    }
            ...
            // Check if the message contains landscape
            if (messageJson.has("landscape")) {
                // Get the landscape value
                String landscapeValue = messageJson.get("landscape").asText();

                // Check if the landscape value is valid
                if (!validLandscapeValues.contains(landscapeValue)) {
                    // Replace the invalid value with "OTHER"
                    ((ObjectNode) messageJson).put("landscape", "OTHER");
                }
            }
            ...
            if (messageJson.has("temperatureList")) {
                // Get the list of transports for the current city
                ArrayNode temperatureList = (ArrayNode) messageJson.get("temperatureList");
                ArrayNode validTemperatureList = mapper.createArrayNode();
                ...
                // Replace the transportList with the new valid list
                ((ObjectNode) messageJson).set("temperatureList", validTemperatureList);
            }
